...
stream = "https://stream60.forexpros.com:443";
...
new_conn = function (){
	...
	on("tick",function(e,data) {
	    ...
	    // $('.pid-' + pid_obj.pid + '-arrowSmall').removeClass('greenFont');
	    // $('.pid-' + pid_obj.pid + '-arrowSmall').removeClass('redFont');
	    // $('.pid-' + pid_obj.pid + '-arrowSmall').removeClass('grayArrowIcon');
	    // if (pid_obj.pc_col == 'greenFont'){$('.pid-' + pid_obj.pid + '-arrowSmall').addClass('greenFont');
	    // }else if (pid_obj.pc_col == 'redFont'){
	    	// $('.pid-' + pid_obj.pid + '-arrowSmall').addClass('redFont');
	    ...
	    if ( (pid_obj.last_dir == 'redBg') || (pid_obj.last_dir == 'greenBg')){
			if(pid_obj.last_dir == 'redBg'){
				$('.pid-' + pid_obj.pid + '-arrowSmall').removeClass('greenFont');
				$('.pid-'+pid_obj.pid+'-last').removeClass('greenBg');
				$('.pid-'+pid_obj.pid+'-bid').removeClass('greenBg');
				$('.pid-'+pid_obj.pid+'-ask').removeClass('greenBg');
			}else if(pid_obj.last_dir == 'greenBg'){
				$('.pid-' + pid_obj.pid + '-arrowSmall').removeClass('redFont');
				$('.pid-'+pid_obj.pid+'-last').removeClass('redBg');
				$('.pid-'+pid_obj.pid+'-bid').removeClass('redBg');
				$('.pid-'+pid_obj.pid+'-ask').removeClass('redBg');
			}
			if(pid_obj.last_dir == 'redBg'){
				$('.pid-' + pid_obj.pid + '-arrowSmall').addClass('redFont');
			}else{
				$('.pid-' + pid_obj.pid + '-arrowSmall').addClass('greenFont');
			}
...
function sendMessage(){
    ...
    // The object to send
    var send = {message: message,username: username};
    // Send it now
